...
<html>
  <head>
	...
	$(function(){
		...
		extrawork.checkExtraWorkForm = $('#checkExtraWorkForm').form();
		//查看所有加班条
			extrawork.all_datagrid = $('#extrawork_datagrid').datagrid({
  				url : sy.bp()+'/extraWork.do?method=get&no=check',
  				pagination : true,
				pageSize : 15,
				pageList : [15,30],
				fit : true,
				...
				columns : [ [ {
					...
					toolbar : ['-',{
							...
							handler : function(){
								...
								if(rows.length > 0) {
									$.messager.confirm('请确认','你确认删除选中项？',function(b){
										if(b){
											var nos = [];
											...
											$.ajax({
												url : sy.bp()+'/extraWork.do?method=del',
												data : {
													nos : nos.join(',')
												},
												dataType : 'json',
												success : function(r){
													...
													extrawork.all_datagrid.datagrid('unselectAll'),
													$.messager.show({
														title : '提示',
														msg : '删除成功！'
													});
									...
									$.messager.alert('提示','请选择要删除的加班！','warning');
								}
							}
						},'-',{
							text : '修改',
							...
							handler : function(){
								editRow = undefined;
								extrawork.all_datagrid.datagrid('rejectChanges');
								extrawork.all_datagrid.datagrid('unselectAll');
							}
						},'-',' ',' ','-',{
							text : '通过',
			...
			extrawork.searchExtrawork = function() {
				extrawork.all_datagrid.datagrid('load',sy.serializeObject(extrawork.checkExtraWorkForm));
			};
			//修改加班dialog
  			extrawork.editDialog = $('#extrawork_editDialog').dialog({
				title : '修改加班',
				iconCls : 'icon-edit',
				modal:true,
				closed : true,
				...
				buttons : [{
					...
					plain : true,
					handler : function(){
						extrawork.startTime = $('#extrawork_editForm').find('input[name=extra_date1]').val();
						extrawork.endTime = $('#extrawork_editForm').find('input[name=extra_date2]').val();
					...
					plain : true,
					handler : function(){
						extrawork.editForm.find('input').val('');
					}
				}],
				onOpen : function() {
					setTimeout(function() {
		    ...
		    //审核加班
			extrawork.check = function(passNo){
  				var rows = extrawork.all_datagrid.datagrid('getSelections');
				if(rows.length == 1) {
					$.ajax({
						url : sy.bp()+'/extraWork.do?method=check&pass='+passNo,
						data : {
							query_id : rows[0].query_id
						},
						dataType : 'json',
  ...
  <body style="background-color: #F5F5F5" class="easyui-layout" fit="true" border="false">
   <div region="north" title="" split="true" style="height: 40px;overflow: hidden;" border="false">
       <form name="checkExtraWorkForm" id="checkExtraWorkForm" action="" method="post">
         ...
         <table>
           <tr>
             ...
             <td>&nbsp;&nbsp;&nbsp;</td> 
             <td>开始日期：</td>
             <td class="calendarBox"><input id="startTime" name="startDate" type="text" style="width: 120px; "/></td>
           
            <td>&nbsp;&nbsp;</td>
            
               
             <td>结束日期：</td>
             <td class="calendarBox"><input id="endTime" name="endDate" type="text" style="width: 120px;"/></td>
            
	          
             <td>&nbsp;&nbsp;&nbsp;</td>   
	         
	         <td><input type="button" onclick="extrawork.searchExtrawork();"  value="查询" /></td>
